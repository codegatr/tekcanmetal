...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && csrf_check()) {
    ...
    $data = i18n_post_merge($data, ['title', 'meta_title', 'excerpt', 'content', 'meta_desc']);
    ...
    <div>
      <div class="adm-panel">
        ...
        <div class="adm-panel-body">
          ...
          <div class="row"><label>Özet</label><textarea name="excerpt" rows="2" maxlength="500"><?= h($row['excerpt'] ?? '') ?></textarea><?= i18n_inputs($row, 'excerpt', true, 3) ?></div>
          <div class="row">
            <label>İçerik</label>
            <textarea name="content" rows="20" id="postContent"><?= h($row['content'] ?? '') ?></textarea><?= i18n_inputs($row, 'content', true, 12) ?>
          </div>
        </div>
      </div>
    </div>
